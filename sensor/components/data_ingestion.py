...
class DataIngestion:
    ...
    def export_data_into_feature_store(self) -> pd.DataFrame:
        ...
        try:
            ...
            logging.info(
                f"Exported dataframe shape: {dataframe.shape} "
                f"rows: {len(dataframe)}, cols: {len(dataframe.columns)}"
            )
            if len(dataframe) == 0:
                raise Exception(
                    f"MongoDB collection '{self.data_ingestion_config.collection_name}' "
